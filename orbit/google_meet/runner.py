...
from .join_flow import attempt_fast_guest_join, classify_join_failure, ensure_joined, get_meeting_status
...
def _log_stage(log_session_id, event, **fields):
    details = " ".join(
        f"{key}={value}" for key, value in fields.items() if value is not None
    )
    message = f"evt={event}" + (f" {details}" if details else "")
    log(message, log_session_id, level="important")
...
async def run_meeting_session(config, callbacks=None, state=None):
    ...
    try:
        ...
        fast_join_enabled = env_bool("GMEET_FAST_JOIN_ENABLED", True)
        fast_join_completed = False
        ...
        if fast_join_enabled:
            await browser.start()
            page = await browser.new_page()
            fast_join_completed = await attempt_fast_guest_join(
                page,
                config.meet_url,
                config.display_name,
                state.session_id,
                timeout_ms=env_int("GMEET_FAST_JOIN_TIMEOUT_MS", 12000),
            )
            state.browser_use_success = fast_join_completed

        if not fast_join_completed:
        ...
        async def notify_waiting_for_host(_status):
            ...
            _log_stage(state.session_id, "join.waiting_for_host")
        ...
        joined, final_join_status = await ensure_joined(
            ...
            poll_interval_ms=env_int("GMEET_JOIN_POLL_INTERVAL_MS", None) or None,
        )
        if joined:
                ...
                if capture_strategy == "server_audio_sink":
                    ...
                    if not state.live_stt_available:
                        ...
                        if state.audio_capture_routing_mode == AUDIO_ROUTING_MODE_PROCESS_ENV:
                            _log_stage(
                                state.session_id,
                                "stt.capture_requested",
                                strategy=state.audio_capture_strategy,
                                mode=state.audio_capture_routing_mode,
                                sink=state.audio_sink_name,
                            )
                            await emit_status(
                                callbacks,
                                state,
                    ...
                    if state.live_stt_available:
                        ...
                        _log_stage(
                            state.session_id,
                            "stt.capture_requested",
                            strategy=state.audio_capture_strategy or "chrome_extension",
                            available=False,
                        )
                        await emit_status(
                            callbacks,
            ...
            _log_stage(state.session_id, "join.failed", reason=join_status)
            log(
                f"{detail} Keeping browser open for {config.wait_after_join_ms // 1000} seconds.",
                state.session_id,
                level="important",
        ...
        _log_stage(state.session_id, "session.failed", error=str(error))
    finally:
